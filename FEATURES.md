...
This document serves as a complete catalog of every feature currently built into the NovaStream v1.0.0 desktop application.
...
- **Franchise Hub (Watch Order)**: Inside an anime's details, it maps out Sequels, Spin-offs, and Side Stories so users know exactly what to watch next.
...
- **Cast & Voice Actors**: Displays the top characters and their Japanese Voice Actors for every show (via Jikan API).
...
- **Smart Recommendations Engine**: Analyzes the user's local library, determines their most-watched genre, and suggests highly-rated anime they *haven't* seen yet.
...
- **Desktop Wrapper**: Built using `pywebview` for a lightweight, Chromium-based standalone desktop application.
- **Python Flask Backend**: Handles API routing, database transactions, and background threading.
- **Compiled Installer**: Packaged into a single `NovaStream_Setup.exe` via PyInstaller and Inno Setup, dynamically injecting `deno.exe` into the Windows PATH at runtime for dependencies.
